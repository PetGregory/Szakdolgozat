...
export class WorkoutsComponent implements OnInit, OnDestroy {
  ...
  async loadExistingWorkout() {
    if (!this.currentUser) return;
    
    try {
      const user = await this.userService.getUser(this.currentUser.uid);
      if (user && user.workout) {
        this.generatedPlan = user.workout;
        
        if (user.gender) this.userData.gender = user.gender;
        if (user.age !== null && user.age !== undefined) this.userData.age = user.age;
        if (user.weight !== null && user.weight !== undefined) this.userData.weight = user.weight;
        if (user.height !== null && user.height !== undefined) this.userData.height = user.height;
        if (user.goal) this.userData.goal = user.goal;
        if (user.fitnessLevel) this.userData.fitnessLevel = user.fitnessLevel;
        if (user.availableDays !== null && user.availableDays !== undefined) this.userData.availableDays = user.availableDays;

        if (this.generatedPlan && (this.generatedPlan.calorieTarget === undefined || this.generatedPlan.calorieTarget === null)) {
          if (user.calorieTarget) {
            this.generatedPlan.calorieTarget = user.calorieTarget;
          } else {
        ...
        setTimeout(() => {
          const workoutPlanElement = document.querySelector('[data-workout-plan]');
          if (workoutPlanElement) {
            workoutPlanElement.scrollIntoView({ behavior: 'smooth', block: 'start' });
          }
        }, 300);
      }
    } catch (error) {
